...
GetShared = () as any =>
    let
        GlobalsTable=Table.FromRecords({#shared})
    in
        GlobalsTable;
GetDevices = (website as text, token as text) =>
    ...
    in
        json;
 GenerateTable = (json as record) =>
     let
         list = Record.Field(json,"results"),
         source = Table.FromRecords(list)
    in
        source;
...
GetComputerAttributes = (id as number,baseurl as text) as any =>
    let 
        ...
        computer=jsonComputerDetails[computer], 
        extensionAttributes=computer[extension_attributes],
        //extensionAttribute=extensionAttributes[extension_attribute]{0},
...
MobileDevicesApps = (baseurl as text) as table =>
   let
       ...
       jsonDevicesGroups=Json.Document(response),
       deviceGroups=Table.FromRecords(jsonDevicesGroups[mobile_device_groups])
      // devicesModified=Table.AddColumn(devices,"mobileDeviceDetails", each testFunc([id]))
    in
        deviceGroups;
...
JamfPro.Publish = [
    Beta = true,
    Category = "Other",
    ButtonText = { "Jamf Pro Connector", "Access your organization JamfPro model" },
